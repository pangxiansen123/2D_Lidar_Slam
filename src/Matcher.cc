...
//构造函数，可以指定参数
Matcher::Matcher(double _r,double _h,int _iter)
{
    m_r = _r;
    m_h = _h;
    m_Iterations = _iter;
...
//设定迭代次数
void Matcher::setIterations(int _iter)
{
    m_Iterations = _iter;
}
...
//设置目标点云（前一帧的点云）
void Matcher::setTargetPointCloud(std::vector<Eigen::Vector2d> &_target_pcloud)
{
 ...
 */
void Matcher::projSourcePtToSurface(
...
bool Matcher::SolveMotionEstimationProblem(std::vector<Eigen::Vector2d> &source_cloud,
 ...
 * @return
 */
Eigen::Vector2d Matcher::ComputeNormal(std::vector<Eigen::Vector2d> &nearPoints)
{
    Eigen::Vector2d normal;
...
bool Matcher::SolverFourthOrderPolynomial(Eigen::VectorXd&p_coffi,
                                                 double &lambda)
{
